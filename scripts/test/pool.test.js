      ...
      JSON.stringify([
        {
          ...
          result: {
            isvalid: true,
            address: 'tb1qprvwwfr5cey54e4353t9dmker7zd9w4uhvkz5p',
          },
        },
        { id: 'nocktest', error: null, result: { networkhashps: 0 } },
        { id: 'nocktest', error: true, result: { code: -1 } },
        {
          id: 'nocktest',
          error: null,
          result: { chain: 'test', difficulty: 0 },
        },
        {
          id: 'nocktest',
          error: null,
...
describe('Test pool functionality', () => {
  ...
  test('Test pool daemon events [4]', done => {
    ...
    pool.on('log', (type, text) => {
      expect(type).toBe('error');
      expect(text).toBe(
        'Failed to connect daemon(s): [{"error":true,"response":null,"instance":{"host":"127.0.0.1","port":8332,"user":"","password":"","index":0}}]'
      );
      done();
    });
    pool.setupDaemonInterface(() => {});
  ...
  test('Test pool batch data events [3]', done => {
    ...
    mockSetupDaemon(pool, () => {
      ...
      pool.setupPoolData(() => {
        expect(optionsCopy.primary.address).toBe(
  ...
  test('Test pool batch data events [4]', done => {
    ...
    pool.on('log', (type, text) => {
      expect(type).toBe('error');
      expect(text).toBe(
        'Could not start pool, error with init RPC call: validateaddress - true'
      );
      done();
    });
    mockSetupDaemon(pool, () => {
        ...
        .reply(
          ...
          JSON.stringify([
            ...
            { id: 'nocktest', error: true, result: { code: -1 } },
            {
              id: 'nocktest',
              error: null,
              result: { chain: 'test', difficulty: { 'proof-of-work': 0 } },
            },
            {
              id: 'nocktest',
              error: null,
  ...
  test('Test pool manager events [3]', done => {
    ...
    pool.on('log', (type, text) => {
      expect(type).toBe('error');
      expect(text).toBe(
        'RPC error with primary daemon instance 0 when submitting block with submitblock true'
      );
      done();
    });
    mockSetupDaemon(pool, () => {
  ...
  test('Test pool manager events [4]', done => {
    ...
    pool.on('log', (type, text) => {
      expect(type).toBe('error');
      expect(text).toBe(
        'Primary daemon instance 0 rejected a supposedly valid block'
      );
      done();
    });
    mockSetupDaemon(pool, () => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 3) {
        ...
        expect(response[0][1]).toBe(
          "Submitted primary block successfully to Bitcoin's daemon instance(s)"
        );
  ...
  test('Test pool manager events [10]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 3) {
        ...
        expect(response[2][1]).toBe(
          'getblocktemplate call failed for daemon instance 0 with error true'
        );
        done();
      }
    });
  ...
  test('Test pool blockchain events [2]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][1]).toBe(
          'Downloaded 100.00% of blockchain from 1 peers'
        );
        done();
      }
    });
  ...
  test('Test pool polling events [1]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[0][1]).toBe(
          'Network diff of 0 is lower than port 3001 w/ diff 32'
        );
  ...
  test('Test pool polling events [2]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('debug');
        expect(response[1][1]).toBe(
          'Primary chain (Bitcoin) notification via RPC polling at height 2'
        );
        done();
      }
    });
  ...
  test('Test pool peer events [1]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('debug');
        expect(response[1][1]).toBe(
          'p2p has been disabled in the configuration'
        );
        done();
      }
    });
  ...
  test('Test pool peer events [2]', done => {
    ...
    optionsCopy.primary.recipients[0].address =
      'tb1qnc0z4696tusrgscws5gvc7g2hhz99m6lrssfc2';
    const pool = new Pool(optionsCopy, null, () => {});
    pool.on('log', (type, text) => {
      response.push([type, text]);
      if (response.length === 2) {
  ...
  test('Test pool peer events [8]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 3) {
        ...
        expect(response[2][1]).toBe(
          'Block template for Bitcoin updated successfully'
        );
        done();
      }
    });
  ...
  test('Test pool peer events [9]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 4) {
        ...
        expect(response[3][1]).toBe(
          'Block notify error getting block template for Bitcoin'
        );
        done();
      }
    });
  ...
  test('Test pool stratum events [2]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('debug');
        expect(response[1][1]).toBe(
          'No new blocks for 60 seconds - updating transactions & rebroadcasting work'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [3]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 4) {
        ...
        expect(response[3][1]).toBe(
          'Updated existing job for current block template'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [4]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 4) {
        ...
        expect(response[3][0]).toBe('debug');
        expect(response[3][1]).toBe(
          'Established new job for updated block template'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [5]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('warning');
        expect(response[1][1]).toBe(
          'Malformed message from client [example]: "Message"'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [6]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('warning');
        expect(response[1][1]).toBe(
          'Socket error from client [example]: "Error"'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [7]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('warning');
        expect(response[1][1]).toBe(
          'Connection timed out for client [example]: Timeout'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [9]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('debug');
        expect(response[1][1]).toBe(
          'Rejected incoming connection from 127.0.0.1. The client is banned for 100000 seconds.'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [11]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('debug');
        expect(response[1][1]).toBe(
          'Unknown stratum method from client [example]: Unknown'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [12]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('warning');
        expect(response[1][1]).toBe(
          'Detected socket flooding from client [example]'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [14]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('warning');
        expect(response[1][1]).toBe(
          'Ban triggered for client [example]: Socket flooding'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [15]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('debug');
        expect(response[1][1]).toBe(
          'Difficulty updated successfully for worker: worker1 (100000)'
        );
        pool.stratum.stopServer();
      }
    });
  ...
  test('Test pool stratum events [16]', done => {
    ...
    pool.on('log', (type, text) => {
      ...
      if (response.length === 2) {
        ...
        expect(response[1][0]).toBe('debug');
        expect(response[1][1]).toBe(
          'Client successfully subscribed to stratum network'
        );
        pool.stratum.stopServer();
      }
    });
