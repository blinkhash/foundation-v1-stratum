...
const Manager = function (options) {
  ...
  this.processTemplate = function (rpcData, processNew) {
    ...
    if (
      ...
      isNewBlock = true;
      if (rpcData.height < _this.currentJob.rpcData.height) isNewBlock = false;
    }
  ...
  this.processShare = function (
    ...
    switch (_this.options.primary.coin.algorithms.mining) {
      ...
      case 'kawpow':
        ...
        if (
          submission.nonce.indexOf(submission.extraNonce1.substring(0, 4)) !== 0
        ) {
        ...
        headerBuffer = job.serializeHeader(
          merkleRoot,
          nTime,
          submission.nonce,
          version
        ...
        headerBigNum = bignum.fromBuffer(hashOutputBuffer, {
          endian: 'big',
          size: 32,
        });
        ...
        shareDiff =
          (Algorithms[algorithm].diff / headerBigNum.toNumber()) *
          shareMultiplier;
      ...
      default:
        ...
        // Generate Coinbase Buffer
        coinbaseBuffer = job.serializeCoinbase(
          extraNonce1Buffer,
          extraNonce2Buffer
        );
        coinbaseHash = job.coinbaseHasher(coinbaseBuffer);
        ...
        headerBigNum = bignum.fromBuffer(headerHash, {
          endian: 'little',
          size: 32,
        });
        ...
        blockDiffAdjusted = job.difficulty * shareMultiplier;
        blockHex = job
          .serializeBlock(headerBuffer, coinbaseBuffer, null, null)
          .toString('hex');
        blockHash = job
          .blockHasher(headerBuffer, submission.nTime)
          .toString('hex');
