...
const Pool = function (options, authorizeFn, responseFn) {
  ...
  this.setDifficulty = function (port, difficultyConfig) {
    ...
    const difficultyInstance = new Difficulty(
      currentPort,
      difficultyConfig,
      false
    );
  ...
  this.setupDaemonInterface = function (callback) {
    if (
      !Array.isArray(_this.options.primary.daemons) ||
      _this.options.primary.daemons.length < 1
    ) {
      emitErrorLog(
        'No primary daemons have been configured - pool cannot start'
      );
      return;
    }
    _this.primary.daemon = _this.setupDaemon(
      _this.options.primary.daemons,
      () => {}
    );
    if (_this.options.auxiliary && _this.options.auxiliary.enabled) {
      if (
        !Array.isArray(_this.options.auxiliary.daemons) ||
        _this.options.auxiliary.daemons.length < 1
      ) {
        emitErrorLog(
          'No auxiliary daemons have been configured - pool cannot start'
        );
        return;
      }
      _this.auxiliary.daemon = _this.setupDaemon(
        _this.options.auxiliary.daemons,
        callback
      );
    } else {
      callback();
    ...
    _this.primary.daemon.batchCmd(batchRPCCommand, (error, results) => {
      ...
      results.forEach((output, idx) => {
        ...
        if (rpcCall !== 'submitblock' && (output.error || !output.result)) {
          emitErrorLog(
            `Could not start pool, error with init RPC call: ${rpcCall} - ${JSON.stringify(
              output.error
            )}`
          );
          return;
      ...
      if (
        _this.options.primary.coin.staking &&
      ...
      if (_this.options.primary.coin.getinfo) {
        _this.options.settings.testnet =
          rpcResults.getinfo.testnet === true ? true : false;
      } else {
        _this.options.settings.testnet =
          rpcResults.getblockchaininfo.chain === 'test' ? true : false;
      }

      // Establish Coin Protocol Version
      _this.options.primary.address = rpcResults.validateaddress.address;
      _this.options.settings.protocolVersion = _this.options.primary.coin
        .getinfo
        ? rpcResults.getinfo.protocolversion
        : rpcResults.getnetworkinfo.protocolversion;
      let difficulty = _this.options.primary.coin.getinfo
      ...
      _this.options.statistics = {
        connections: _this.options.primary.coin.getinfo
          ? rpcResults.getinfo.connections
          : rpcResults.getnetworkinfo.connections,
        difficulty:
          difficulty *
          Algorithms[_this.options.primary.coin.algorithms.mining].multiplier,
      };
  ...
  this.submitBlock = function (blockHex, callback) {
    ...
    _this.primary.daemon.cmd(rpcCommand, rpcArgs, results => {
      for (let i = 0; i < results.length; i += 1) {
        ...
        if (result.error) {
          emitErrorLog(
            'RPC error with primary daemon instance ' +
          ...
          emitErrorLog(
            `Primary daemon instance ${result.instance.index} rejected a supposedly valid block`
          );
          return;
        }
      }
      emitSpecialLog(
        `Submitted primary block successfully to ${_this.options.primary.coin.name}'s daemon instance(s)`
      );
      callback();
    });
  };
  ...
  this.submitAuxBlock = function (
    ...
    _this.auxiliary.daemon.cmd('getauxblock', rpcArgs, results => {
      for (let i = 0; i < results.length; i += 1) {
        ...
        if (result.error) {
          emitErrorLog(
            'RPC error with auxiliary daemon instance ' +
              result.instance.index +
              ' when submitting block with getauxblock ' +
              JSON.stringify(result.error)
          );
          return;
        } else if (!result.response || result.response === 'rejected') {
          emitErrorLog(
            `Auxiliary daemon instance ${result.instance.index} rejected a supposedly valid block`
          );
          return;
        }
      }
      emitSpecialLog(
        `Submitted auxiliary block successfully to ${_this.options.auxiliary.coin.name}'s daemon instance(s)`
      );
    ...
    _this.primary.daemon.cmd(
      ...
      result => {
        if (result.error) {
          ...
          const processedNewBlock = _this.manager.processTemplate(
            result.response,
            force
          );
  ...
  this.getAuxTemplate = function (callback) {
    if (_this.options.auxiliary && _this.options.auxiliary.enabled) {
      _this.auxiliary.daemon.cmd('getauxblock', [], result => {
        if (result[0].error) {
          emitErrorLog(
            'getauxblock call failed for daemon instance ' +
          ...
          const hash =
            result[0].response.target || result[0].response._target || '';
  ...
  this.setupJobManager = function () {
    ...
    _this.manager.on('share', (shareData, auxShareData, blockValid) => {
      ...
      if (!shareData.error) {
        shareType = { valid: true, stale: false, invalid: false };
      } else if (shareData.error == 'stale share') {
        shareType = { valid: false, stale: true, invalid: false };
      } else {
        shareType = { valid: false, stale: false, invalid: true };
      }
          ...
          _this.checkBlockAccepted(
            ...
            (accepted, tx) => {
              ...
              _this.getBlockTemplate((error, result, foundNewBlock) => {
                if (foundNewBlock) {
                  emitSpecialLog(
                    'Block notification via RPC after primary block submission'
                  );
                }
              }, false);
            }
      ...
      if (
        shareType.valid &&
        _this.options.auxiliary &&
        _this.options.auxiliary.enabled
      ) {
        ...
        const difficulty = parseFloat(
          (
            Algorithms[algorithm].diff /
            _this.auxiliary.rpcData.target.toNumber()
          ).toFixed(9)
        );
        ...
        if (_this.auxiliary.rpcData.target.ge(auxShareData.headerDiff)) {
          ...
          _this.submitAuxBlock(
            hexBuffer,
            auxShareData.coinbase,
            auxShareData.header,
            hash => {
              _this.checkBlockAccepted(
                hash,
                ...
                (accepted, tx) => {
                  ...
                  _this.getBlockTemplate((error, result, foundNewBlock) => {
                    if (foundNewBlock) {
                      emitSpecialLog(
                        'Block notification via RPC after auxiliary block submission'
                      );
                    }
                  }, true);
                }
  ...
  this.setupBlockchain = function (callback) {
    ...
    const generateProgress = function () {
      const cmd = _this.options.primary.coin.getinfo
        ? 'getinfo'
        : 'getblockchaininfo';
      _this.primary.daemon.cmd(cmd, [], results => {
        const blockCount = Math.max.apply(
          null,
          results
        ...
        _this.primary.daemon.cmd('getpeerinfo', [], results => {
          ...
          const totalBlocks = Math.max.apply(
            null,
            peers.flatMap(response => response.startingheight)
          );
    ...
    checkSynced(() => {
      limitMessages(() => {
        emitErrorLog(
          'Daemon is still syncing with the network. The server will be started once synced'
        );
      });
    });
  };
    ...
    _this.getAuxTemplate(() => {
      _this.getBlockTemplate(error => {
        ...
        if (portWarnings.length > 0) {
          limitMessages(() => {
            emitWarningLog(
              `Network diff of ${networkDiffAdjusted} is lower than ${portWarnings.join(
                ' and '
              )}`
            );
          });
  ...
  this.setupBlockPolling = function () {
    if (
      typeof _this.options.settings.blockRefreshInterval !== 'number' ||
      _this.options.settings.blockRefreshInterval <= 0
    ) {
    ...
    setInterval(() => {
      if (pollingFlag === false) {
        pollingFlag = true;
        _this.getAuxTemplate(
          (auxililaryError, auxiliaryResult, auxiliaryUpdate) => {
            _this.getBlockTemplate(
              (primaryError, primaryResult, primaryUpdate) => {
                pollingFlag = false;
                if (primaryUpdate && !auxiliaryUpdate) {
                  limitMessages(() => {
                    emitLog(
                      `Primary chain (${_this.options.primary.coin.name}) notification via RPC polling at height ${primaryResult.height}`
                    );
                  });
                }
                if (auxiliaryUpdate) {
                  limitMessages(() => {
                    emitLog(
                      `Auxiliary chain (${_this.options.auxiliary.coin.name}) notification via RPC polling at height ${auxiliaryResult.height}`
                    );
                  });
                }
              },
  ...
  this.processBlockNotify = function (blockHash) {
    ...
    if (
      ...
      blockHash !== currentJob.rpcData.previousblockhash
    ) {
      _this.getBlockTemplate(error => {
        if (error) {
          emitErrorLog(
            `Block notify error getting block template for ${_this.options.primary.coin.name}`
          );
        } else {
          emitLog(
            `Block template for ${_this.options.primary.coin.name} updated successfully`
          );
        }
      }, false);
    }
  ...
  this.setupPeer = function () {
    ...
    if (
      _this.options.settings.testnet &&
      !_this.options.primary.coin.testnet.peerMagic
    ) {
      emitErrorLog(
        'p2p cannot be enabled in testnet without peerMagic set in testnet configuration'
      );
      return;
    } else if (!_this.options.primary.coin.mainnet.peerMagic) {
      emitErrorLog(
        'p2p cannot be enabled without peerMagic set in mainnet configuration'
      );
      return;
    }
  ...
  this.setupStratum = function (callback) {
    ...
    _this.stratum.on('broadcastTimeout', () => {
      if (_this.options.debug) {
        emitLog(
          `No new blocks for ${_this.options.settings.jobRebroadcastTimeout} seconds - updating transactions & rebroadcasting work`
        );
      }
    ...
    _this.stratum.on('client.connected', client => {
      ...
      client.on('difficultyQueued', diff => {
        emitLog(
          'Difficulty update queued for worker: ' +
            client.addrPrimary +
            ' (' +
            diff +
            ')'
      ...
      client.on('difficultyChanged', diff => {
        emitLog(
          'Difficulty updated successfully for worker: ' +
            client.addrPrimary +
            ' (' +
            diff +
            ')'
      ...
      client.on('malformedMessage', message => {
        emitWarningLog(
          `Malformed message from ${client.getLabel()}: ${JSON.stringify(
            message
          )}`
        );
      });
      client.on('socketError', e => {
        emitWarningLog(
          `Socket error from ${client.getLabel()}: ${JSON.stringify(e)}`
        );
      });
      client.on('socketTimeout', reason => {
        emitWarningLog(
          `Connection timed out for ${client.getLabel()}: ${reason}`
        );
      });
      ...
      client.on('kickedBannedIP', remainingBanTime => {
        emitLog(
          `Rejected incoming connection from ${client.remoteAddress}. The client is banned for ${remainingBanTime} seconds.`
        );
      });
      ...
      client.on('unknownStratumMethod', fullMessage => {
        emitLog(
          `Unknown stratum method from ${client.getLabel()}: ${
            fullMessage.method
          }`
        );
      });
  ...
  this.outputPoolInfo = function () {
    ...
    if (
      ...
      infoLines.push(
        `Block Polling Every:\t${_this.options.settings.blockRefreshInterval} ms`
      );
    }
    limitMessages(() => {
