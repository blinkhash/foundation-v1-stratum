...
const Network = function(options, portalOptions, authorizeFn) {
  ...
  this.setupNetwork = function() {
    ...
    stratumPorts.forEach((port) => {
      ...
      if (enabled) {
        const serverKey = fs.readFileSync(path.join('./certificates', _this.portalOptions.tls.serverKey));
        const serverCert = fs.readFileSync(path.join('./certificates', _this.portalOptions.tls.serverCert));
      }
